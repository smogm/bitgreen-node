...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/bitgreen-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update bitgreen-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'bitgreen-lib': '^v0.15.4',
          '@bitgreen/bitgreen-node': '^v3.0.7'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/bitgreen-node.json');
