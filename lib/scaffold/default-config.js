...
/**
 * Will return the path and default bitgreen-node configuration. It will search for the
 * configuration file in the "~/.bitgreen" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.bitgreen');
  var defaultConfigFile = path.resolve(defaultPath, './bitgreen-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['bitgreend', 'web'];
