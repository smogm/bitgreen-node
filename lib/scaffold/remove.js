...
/**
 * Will remove a service from bitgreen-node.json
...
/**
 * Will remove the Node.js service and from the bitgreen-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The bitgreen-node configuration directory
...
function remove(options, done) {
  ...
  var bitgreenConfigPath = path.resolve(configPath, 'bitgreen-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitgreenConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a bitgreen-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to bitgreen-node.json
        removeConfig(bitgreenConfigPath, service, next);
      });
    }, done
  );
}
