...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitgreen-lib': 'https://github.com/bitgreen/bitgreen-lib',
    'bitgreen-node': 'https://github.com/bitgreen/bitgreen-node'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitgreen-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the bitgreen node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitgreen-node directory and configuration
