...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitgreend is necessary for this upgrade with the "reindex=1" bitgreen.conf option. \n' +
                 'There are changes necessary in both dash.conf and bitgreen-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitgreen/bitgreen-node/blob/bitgreend/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitgreend: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/bitgreend')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed bitgreen-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitgreen-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bitgreen-node services directory
    ...
    if (servicePackage.bitgreenNode) {
      serviceModule = service.name + '/' + servicePackage.bitgreenNode;
    }
