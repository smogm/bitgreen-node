...
Dash.prototype.getInfo = function(callback) {
  ...
  this.client.getNetworkInfo(function(err, response) {
    ...
    var networkInfoResult = response.result;
    self.client.getBlockchainInfo(function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      var blockchainInfoResult = response.result;
      var info = {
        version: networkInfoResult.version,
        protocolVersion: networkInfoResult.protocolversion,
        blocks: blockchainInfoResult.blocks,
        timeOffset: networkInfoResult.timeoffset,
        connections: networkInfoResult.connections,
        proxy: networkInfoResult.networks[0].proxy, // workaround: first index may be invalid?
        difficulty: blockchainInfoResult.difficulty,
        testnet: (blockchainInfoResult.chain === 'testnet' ? true : false),
        relayFee: networkInfoResult.relayfee,
        errors: '',
